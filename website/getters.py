...
def get_orders():
  ...
  for id in ids:
    ...
    grabber = {'id': 0, 'session_id': '', 'options': [], 'customer_name': '', 'name': '', 'quantity': 0, 'stat': 1}
    grabber['id'] = order.id
    ...
    try:
      if grabber['session_id'] in all_orders and grabber['stat'] < 3:
        all_orders[grabber['session_id']] += [{'id': grabber['id'], 'options': grabber['options'],'name': grabber['name'], 'quantity': grabber['quantity'], 'stat': grabber['stat'], 'customer_name': grabber['customer_name']}]
